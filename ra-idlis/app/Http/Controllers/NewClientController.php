...
class NewClientController extends Controller {
	...
	public function __applyAttach(Request $request, $hfser, $appid, $office = 'hfsrb') {
		// try {
			...
			$aptid = DB::table('appform')->SELECT('aptid')->where([['appid', $appid]])->first();

			if(!is_null($aptid) && $submitted == true)
			{ 
				$aptid = $aptid->aptid;
			
				if($aptid == "IC")
				{	
					if(FunctionsClientController::isAppIC_ReqFDA($appid) == false) 
					{
...
public function fdacertN(Request $request, $appid, $requestOfClient = null) {
		...
		//{	
			...
			//try {
				if(true) 
				{
					...
					$asc_services_list	= FunctionsClientController::get_view_ServiceList(1, 1);
					...
					$data3 = [
						...
						'asc_services_list'		=> $asc_services_list,
	...
	public function __editApp_submit(Request $request, $hfser, $appid) 
	{
		...
		else if($grp_id == 13)
		{
			...
			DB::table('appform')->where('appid',$appid)->update(['noofbed' => $request->noofbed_applied]);
		}
		...
		else if($grp_id == 14)
		{
			...
			DB::table('appform')->where('appid',$appid)->update(['noofdialysis' => $request->noofdialysis]);
		}
		//For Pharmacy
		else if($grp_id == 15)
		{
			...
			if(count($data) > 0) 
			{
				...
				if($appid > 0) 
				{
					/*$data = DB::table('appform')->select('view_registered_facility_for_change.*','appform.appid')->where('appform.regfac_id', $reg_fac_id)
								->join('view_registered_facility_for_change', 'view_registered_facility_for_change.regfac_id', '=', 'appform.regfac_id')->orderBy('appid','DESC')->get();*/					
