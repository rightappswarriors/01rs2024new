@if (session()->exists('employee_login'))      
  ...
  @section('title', 'Work Status Master File')
  @section('content')
  <input type="text" id="CurrentPage" hidden="" value="PR004">
  <div class="content p-4">
      ...
      <div class="card">
          <div class="card-header bg-white font-weight-bold">
             Work Status <span class="PR004_add"><a href="#" title="Add New Work Status" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>
                ...
                <tbody>
                  ...
                  @foreach ($pwStats as $pwStat)
                    <tr>
                      ...
                      <td>
                        <center>
                          <span class="PR004_update">
                            <button type="button" class="btn btn-outline-warning" onclick="showData('{{$pwStat->pworksid}}', '{{$pwStat->pworksname}}');" data-toggle="modal" data-target="#GodModal"><i class="fa fa-fw fa-edit"></i></button>
                          </span>
                          <span class="PR004_cancel">
                            <button type="button" class="btn btn-outline-danger" onclick="showDelete('{{$pwStat->pworksid}}', '{{$pwStat->pworksname}}');" data-toggle="modal" data-target="#DelGodModal"><i class="fa fa-fw fa-trash"></i></button>
                          </span>
                        </center>
                      </td>
                    </tr>
  ...
  <script type="text/javascript">
      ...
      function deleteNow(){
            ...
            $.ajax({
              url : "{{ asset('employee/mf/del_workstatus') }}",
              method: 'POST',
              data: {_token:$('#token').val(),id:id,mod_id : $('#CurrentPage').val()},
              success: function(data){
                if (data == 'DONE') {
                  alert('Successfully deleted '+name);
                  window.location.href = "{{ asset('/employee/dashboard/mf/workstatus') }}";
                } else if (data == 'ERROR'){
     	...
     	$('#EditNow').on('submit',function(event){
  		...
  		form.parsley().validate();
  		 if (form.parsley().isValid()) {
  		   ...
  		   var y = $('#edit_desc').val();
  		   $.ajax({
  		      url: "{{ asset('employee/mf/save_workstatus') }}",
  		      method: 'POST',
