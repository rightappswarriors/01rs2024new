  ...
  <div class="content p-4">
      ...
      <div class="card">
          <div class="card-header bg-white font-weight-bold">
             Active facilities <span class="TM001_add" ><a href="#" title="Import" data-toggle="modal" data-target="#myModal"><button class="btn btn-primarys"><i class="fa fa-plus-circle" style="cursor: pointer;"></i>Import</button></a></span>
          </div>
          <div class="card-body">
              <table class="table display" id="example" style="overflow-x: scroll;" >
  				<thead>
  					<tr>
  					  <th style="width: 10%">Health Facility Code</th>
  					  <th style="width: 10%">Facility Name</th>
  					  <th style="width: 10%">Health Facility Type</th>
  					  <th style="width: 10%">Ownership</th>
              <th style="width: 10%">Email Address</th>
              <th style="width: 10%">City/Municipality Name</th>
  					</tr>
  				</thead>
  				<tbody>
  					@if (isset($nhfr_table))
  						@foreach ($nhfr_table as $nhfr)
  						<tr>
  							<td scope="row"> {{$nhfr->HealthFacilityCode}}</td>
  							<td>{{$nhfr->FacilityName}}</td>
  							<td>{{$nhfr->HealthFacilityType}}</td>
                <td>{{$nhfr->OwnershipMajorClassification}}</td>
                <td>{{$nhfr->EmailAddress	}}</td>
  							<td>{{$nhfr->CityMunicipalityName}}</td>
    	...
    	<div class="modal-dialog" role="document">
          <div class="modal-content" style="border-radius: 0px;border: none;">
            	<div class="modal-body text-justify" style=" background-color: #272b30;color: white;">
                  <h5 class="modal-title text-center"><strong>Import Active Facility</strong></h5>
                  <hr>
                  <div class="container">
                    <form method="POST" action="{{asset('employee/nhfr/import/nhfr')}}" enctype="multipart/form-data">
                      {{ csrf_field() }}
