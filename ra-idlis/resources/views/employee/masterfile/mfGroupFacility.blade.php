@if (session()->exists('employee_login'))     
  ...
  @section('title', 'Group Facility Master File')
  @section('content')
  <input type="text" id="CurrentPage" hidden="" value="AP014">
  <div class="content p-4">
      <div class="card">
          <div class="card-header bg-white font-weight-bold">
             Group Facility <span class="AP014_add"><a href="" title="Add New" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>  
          </div>
          <div class="card-body">
  ...
  <script type="text/javascript">
  	...
  	function filterGroup(){
          ...
          $.ajax({
                  url: " {{asset('employee/mf/get_facilities')}}",
                  method: 'POST',
                  data: {
                    _token : token,
                    hfser_id : id,
                    ...
                    if (data == 'NONE') {
                      ...
                      for (var i = 0; i < data.length; i++) {
                        ...
                        $('#example').DataTable().row.add([
                              data[i].hgpdesc,
                              '<center><span class="AP014_cancel"><button class="btn btn-outline-danger" data-toggle="modal" data-target="#IfActiveModal" onclick="ShowDelete('+data[i].id+', \''+data[i].hgpdesc+'\', \''+data[i].hfser_desc+'\')"><i class="fa fa-fw fa-trash"></i></button></span></center>'])
                        .draw();
                      }
                      GroupRightsActivate();
                    }
      ...
      $('#NewFacServIn').on('submit',function(event){
        ...
        if (form.parsley().isValid()) {
            $.ajax({
                    ...
                    success: function(data) {
                      if (data) {
                          logActions('Added new Group Facility with ID: '+ data);
                          alert('Successfully Added Type Facility in an Application');
                          window.location.href = "{{ asset('employee/dashboard/mf/group/facilities') }}";
                      } else if (data == 'SAME') {
      ...
      $('#AddOOP').on('submit',function(event){
          ...
          if (form.parsley().isValid()) {
            $.ajax({
                url : '{{ asset('employee/mf/del_facilities') }}',
                method : 'POST',
