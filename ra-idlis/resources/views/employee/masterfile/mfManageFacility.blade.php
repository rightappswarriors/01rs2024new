@if (session()->exists('employee_login'))     
  ...
  @section('content')
  <input type="text" id="CurrentPage" hidden="" value="AP019">
  <div class="content p-4">
      <div class="card">
          <div class="card-header bg-white font-weight-bold">
             Manage Facilities <span class="AP019_add"><a href="" title="Add New" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>  
          </div>
          <div class="card-body">
                    ...
                    if (data == 'NONE') {
                      ...
                      for (var i = 0; i < data.length; i++) {
                        ...
                        $('#example').DataTable().row.add([
                              data[i].hgpdesc,
                              '<center><span class="AP019_cancel"><button class="btn btn-outline-danger" data-toggle="modal" data-target="#IfActiveModal" onclick="ShowDelete('+data[i].tyf_id+', \''+data[i].hgpdesc+'\', \''+data[i].hfser_desc+'\')"><i class="fa fa-fw fa-trash"></i></button></span></center>'])
                        .draw();
                      }
                      GroupRightsActivate();
                    }
      ...
      $('#NewFacServIn').on('submit',function(event){
        ...
        if (form.parsley().isValid()) {
            $.ajax({
                    ...
                    success: function(data) {
                      if (data) {
                          logActions('Added new Manage Facility with ID: '+ data);
