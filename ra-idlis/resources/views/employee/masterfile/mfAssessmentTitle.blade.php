@if (session()->exists('employee_login'))	
	...
	@section('title', 'Assessment Part Master File')
	@section('content')
	...
	<div class="content p-4">
		...
		<div class="card">
			...
			Assessment Part <span class="header_add"><a href="#" title="Add New Assessment Part" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>
				...
				<table class="table display" id="example" style="overflow-x: scroll;" >
					<thead>
						<tr>
							...
							<th class="text-center">User Interface</th>
						...
						@if (!empty($allData))
							@foreach($allData as $titleData)
								<tr class="text-center">
									...
									<td>
										<button type="button" class="btn btn-outline-warning" onclick="showData('{{$titleData->title_code}}', '{{$titleData->title_name}}', '{{$titleData->filename}}', '{{$titleData->facid}}');" data-toggle="modal" data-target="#GodModal"><i class="fa fa-fw fa-edit"></i></button>
										<button type="button" class="btn btn-outline-danger" onclick="showDelete('{{$titleData->title_code}}', '{{$titleData->title_name}}');" data-toggle="modal" data-target="#DelGodModal"><i class="fa fa-fw fa-trash"></i></button>
									</td>
								</tr>
							@endforeach
						@endisset
	...
	<div class="modal fade" id="myModal" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content" style="border-radius: 0px;border: none;">
				...
				color: white;">
					<h5 class="modal-title text-center"><strong>Add New Assessment Part</strong></h5>
					<hr>
					<div class="container">
						<form id="addRgn" class="row"  data-parsley-validate>
							...
							<div class="col-sm-4">Part Code:</div>
							...
							<div class="col-sm-4">Part Description:</div>
							<div class="col-sm-8" style="margin:0 0 .8em 0;">
								<input type="text" id="new_rgn_title" class="form-control" data-parsley-required-message="*<strong>Part Name</strong> required" required>
							</div>
							<div class="col-sm-4">Services:</div>
							<div class="col-sm-8" style="margin:0 0 .8em 0;">
								<select name="newserv" id="newserv" class="form-control" style="width: 100%:">
									<option value disabled readonly hidden="" selected="" value="">Please Select</option>
									@foreach($serv as $se)
									<option value="{{$se->facid}}">{{$se->facid}} - {{$se->facname}}</option>
									@endforeach
								</select>
							</div>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content" style="border-radius: 0px;border: none;">
				<div class="modal-body" style=" background-color: #272b30;color: white;">
					<h5 class="modal-title text-center"><strong>Edit Assessment Part</strong></h5>
					<hr>
					<div>
	...
	<div class="modal fade" id="DelGodModal" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content" style="border-radius: 0px;border: none;">
				<div class="modal-body" style=" background-color: #272b30;color: white;">
					<h5 class="modal-title text-center"><strong>Delete Asessment Part</strong></h5>
					<hr>
	...
	<script type="text/javascript">
		...
		function showData(id,desc, UI,serv){
	      $('#EditBody').empty();
	      $('#EditBody').append(
	          '<div class="col-sm-4">Part Code:</div>' +
	          ...
	          '<div class="col-sm-4">Services:</div>' +
	          '<div class="col-sm-12" style="margin:0 0 .8em 0;">' +
	            '<select name="newserv" id="newserv_edit" class="form-control">'+
					'<option value disabled readonly hidden="" selected="" value="">Please Select</option>'+
					@foreach($serv as $se)
					'<option value="{{$se->facid}}">{{$se->facid}} - {{$se->facname}}</option>'+
					@endforeach
	            ...
	            if (test == -1) { // Not in Array
	                $.ajax({
	                  ...
	                  success: function(data) {
	                    if (data == 'DONE') {
	                        alert('Successfully Added New Assessment Part');
	                        location.reload();
	                    } else if(data == 'ERROR'){
