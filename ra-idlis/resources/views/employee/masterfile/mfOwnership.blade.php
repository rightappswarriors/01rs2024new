@if (session()->exists('employee_login')) 
  ...
  @section('title', 'Ownership Master File')
  @section('content')
  <input type="text" id="CurrentPage" hidden="" value="AP005">
  <div class="content p-4">
      ...
      <div class="card">
          <div class="card-header bg-white font-weight-bold">
             Ownership <span class="AP005_add"><a href="#" title="Add New Ownership" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>
                  ...
                  @if($oShip)
                    @foreach ($oShip as $oShips)
                      <tr>
                        ...
                        <td>
                          <center>
                            <span class="AP005_update">
                              <button type="button" class="btn btn-outline-warning" onclick="showData('{{$oShips->ocid}}', '{{$oShips->ocdesc}}');" data-toggle="modal" data-target="#GodModal"><i class="fa fa-fw fa-edit"></i></button>
                            </span>
                            <span class="AP005_cancel">
                              <button type="button" class="btn btn-outline-danger" onclick="showDelete('{{$oShips->ocid}}', '{{$oShips->ocdesc}}');" data-toggle="modal" data-target="#DelGodModal"><i class="fa fa-fw fa-trash"></i></button>
                            </span>
                          </center>
                        </td>
                      </tr>
  ...
  <script type="text/javascript">
              ...
              if (test == -1) { // Not in Array
                  $.ajax({
                    ...
                    success: function(data) {
                      if (data == 'DONE') {
                          logActions('Added new ownership with ID: '+ $('#new_rgnid').val());
      ...
      $('#EditNow').on('submit',function(event){
  		...
  		form.parsley().validate();
  		 if (form.parsley().isValid()) {
  		   	...
  		   	var y = $('#edit_desc').val();
  		   	$.ajax({
  		     	url: "{{ asset('employee/mf/save_ownership') }}",
  		      	method: 'POST',
  		      	data : {_token:$('#token').val(),id:x,name:y,mod_id : $('#CurrentPage').val()},
  		      	success: function(data){
  		          	if (data == "DONE") {
                      logActions('Edited ownership with ID: '+$('#edit_name').val());
  	...
  	function deleteNow(){
  	    ...
  	    $.ajax({
  	        url : "{{ asset('employee/mf/del_ownership') }}",
  	        method: 'POST',
  	        data: {_token:$('#token').val(),id:id,mod_id : $('#CurrentPage').val()},
  	        success: function(data){
  	          if (data == 'DONE') {
                logActions('Deleted ownership with ID: '+id);
