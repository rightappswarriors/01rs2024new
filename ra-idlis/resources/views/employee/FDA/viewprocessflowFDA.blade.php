                    ...
                    @foreach ($LotsOfDatas as $data)
                        <tr>
                          <td class="text-center" style="width:50px;">          
                              <button type="button" title="View detailed information for {{$data->facilityname}}" class="btn btn-info form-control" onclick="showData({{$data->appid}},'{{$data->aptdesc}}', '{{$data->authorizedsignature}}','{{$data->brgyname}}', '{{$data->classname}}' ,'{{$data->cmname}}', '{{$data->email}}', '{{str_replace(['"',"'"], "",strtoupper($data->facilityname))}}','{{$data->hgpdesc}}', '{{$data->formattedDate}}', '{{$data->formattedTime}}', '{{$data->hfser_desc}}','{{$data->ocdesc}} - {{$data->classname}} - {{$data->subclassname}}', '{{$data->provname}}','{{$data->rgn_desc}}', '{{$data->street_name}}', '{{$data->zipcode}}', '{{$data->isrecommended}}', '{{$data->hfser_id}}', '{{$data->status}}', '{{$data->uid}}', '{{$data->trns_desc}}');" data-toggle="modal" data-target="#GodModal"><i class="fa fa-fw fa-eye"></i></button>
                            
                            @if($FDAtype == 'machines')
                                     
                                <button type="button" title="Update Status for {{str_replace(['"',"'"], "",strtoupper($data->facilityname))}}" class="btn btn-info form-control" onclick="showDataStatus({{$data->appid}},'{{str_replace(['"',"'"], "",strtoupper($data->facilityname))}}', '{{$data->status}}');" data-toggle="modal" data-target="#ModalStatus" style="padding: 5px;margin:1px;font-size: xx-small;">Update Status</button>

                                <button type="button" title="Edit Remarks for {{str_replace(['"',"'"], "",strtoupper($data->facilityname))}}" class="btn btn-info form-control" onclick="showDataRemarks({{$data->appid}},'{{str_replace(['"',"'"], "",strtoupper($data->facilityname))}}', '{{$data->RecoRemarkFDA}}');" data-toggle="modal" data-target="#ModalRemarks" style="padding: 5px;margin:1px;font-size: xx-small;">Edit Remarks</button>
                                
                                <button type="button" title="Upload COC for {{str_replace(['"',"'"], "",strtoupper($data->facilityname))}}" class="btn btn-info form-control" onclick="showDataCOC({{$data->appid}},'{{str_replace(['"',"'"], "",strtoupper($data->facilityname))}}', '{{$data->xrayVal}}', '{{$data->xrayCOC}}', '{{$data->xrayUp}}', '{{$data->xrayUp2}}');" data-toggle="modal" data-target="#ModalCOC" style="padding: 5px;margin:1px;font-size: xx-small;">Upload COC</button>
