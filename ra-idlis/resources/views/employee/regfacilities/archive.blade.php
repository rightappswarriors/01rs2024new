              ...
              @if (isset($LotsOfDatas))
                @foreach ($LotsOfDatas as $data)
                    ...
                    <tr>
                      <td>
                        <center>
                          <a href="{{asset('employee/dashboard/facilityrecords/archive/')}}/{{$data->regfac_id}}" title="View detailed information for {{$data->facilityname}}" class="btn btn-outline-info form-control"><i class="fa fa-fw fa-folder"></i></a>
                        </center>
                      </td>
