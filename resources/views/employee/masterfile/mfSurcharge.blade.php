@if (session()->exists('employee_login'))  
  ...
  @section('title', 'Surcharge Master File')
  @section('content')
  ...
  <div class="content p-4">
      ...
      <div class="card">
          <div class="card-header bg-white font-weight-bold">
             Surcharge <span class="PY001_add"><a href="#" title="Add New Surcharge" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>
          </div>
          <div class="card-body">
                ...
                <tbody>
                  ...
                  @foreach ($surcharges as $surcharge)
                    <tr>
                      <td scope="row"> {{$surcharge->id}}</td>
                      <td>{{$surcharge->date_start}} - {{$surcharge->date_end}}</td>
                      <td>{{$surcharge->description}}</td>
                      <td>{{$surcharge->percentage * -1}}</td>
                      <td>{{$surcharge->type}}</td>
                      <td>{{$surcharge->status == '1' ? 'Active' : 'Inactive'}}</td>
                      <td>
                        <center>
                          <span class="PY001_update">   
                            <button type="button" class="btn btn-outline-warning" onclick="showData('{{$surcharge->id}}', '{{$surcharge->description}}', '{{$surcharge->percentage * -1}}' , '{{$surcharge->type}}', '{{$surcharge->date_start}}', '{{$surcharge->date_end}}', '{{$surcharge->status}}');" data-toggle="modal" data-target="#GodModal"><i class="fa fa-fw fa-edit"></i></button>
                          </span>
                          <span class="PY001_cancel">
                            <button type="button" class="btn btn-outline-danger" onclick="showDelete('{{$surcharge->id}}', '{{$surcharge->description}}');" data-toggle="modal" data-target="#DelGodModal"><i class="fa fa-fw fa-trash"></i></button>
                          </span>
                        </center>
                      </td>
                    </tr>
  ...
  <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog" role="document">
          ...
          color: white;">
              <h5 class="modal-title text-center"><strong>Add Surcharge</strong></h5>
              <hr>
              <div class="container">
  	...
  	<div class="modal-dialog" role="document">
  	  <div class="modal-content" style="border-radius: 0px;border: none;">
  	    <div class="modal-body" style=" background-color: #272b30;color: white;">
  	      <h5 class="modal-title text-center"><strong>Delete Surcharge</strong></h5>
  	      <hr>
  	      <div class="container">
  	        <span id="DelModSpan">
  	        </span>
  ...
  <script type="text/javascript">
      ...
      function deleteNow(){
            ...
            $.ajax({
              url : "{{ asset('employee/mf/del_surcharge') }}",
              method: 'POST',
              data: {_token:$('#token').val(),id:id,mod_id : $('#CurrentPage').val()},
              success: function(data){
                alert('Successfully deleted '+name);
                window.location.href = "{{ asset('/employee/dashboard/mf/surcharge') }}";
              }
            });
          }
            ...
            event.preventDefault();
              ...
              form.parsley().validate();
               if (form.parsley().isValid()) {
                 ...
                 $.ajax({
                    url: "{{ asset('employee/mf/save_surcharge') }}",
                    method: 'POST',
                    data : {
                        _token:$('#token').val(),
                        id:$('#edit_id').val(),
                    ...
                    success: function(data){
                        if (data == "DONE") {
                            alert('Successfully Edited Surcharge');
                            window.location.href = "{{ asset('employee/dashboard/mf/surcharge') }}";
                        }
                    }
                 });
