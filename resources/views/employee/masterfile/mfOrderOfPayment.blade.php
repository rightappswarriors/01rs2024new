@if (session()->exists('employee_login'))  
  ...
  @section('title', 'Order of Payment Master File')
  @section('content')
  ...
  <div class="content p-4">
      ...
      <div class="card">
          <div class="card-header bg-white font-weight-bold">
             Order of Payment <span class="PY001_add"><a href="#" title="Add New Order of Payment" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>
          </div>
          <div class="card-body">
                      ...
                      <td>
                        <center>
                          <span class="PY001_update">
                            <button type="button" class="btn btn-outline-warning" onclick="showData('{{$oop->oop_id}}', '{{$oop->oop_desc}}');" data-toggle="modal" data-target="#GodModal"><i class="fa fa-fw fa-edit"></i></button>
                          </span>
                          <span class="PY001_cancel">
                            <button type="button" class="btn btn-outline-danger" onclick="showDelete('{{$oop->oop_id}}', '{{$oop->oop_desc}}');" data-toggle="modal" data-target="#DelGodModal"><i class="fa fa-fw fa-trash"></i></button>
                          </span>
  ...
  <script type="text/javascript">
      ...
      function deleteNow(){
            ...
            $.ajax({
              url : "{{ asset('employee/mf/del_orderofpayment') }}",
              method: 'POST',
            ...
            event.preventDefault();
              ...
              form.parsley().validate();
               if (form.parsley().isValid()) {
                 ...
                 var y = $('#edit_desc').val();
                 $.ajax({
                    url: "{{ asset('employee/mf/save_orderofpayment') }}",
                    method: 'POST',
                    data : {_token:$('#token').val(),id:x,name:y,mod_id : $('#CurrentPage').val()},
                    success: function(data){
                        if (data == "DONE") {
                            alert('Successfully Edited Order of Payment');
                            window.location.href = "{{ asset('employee/dashboard/mf/orderofpayment') }}";
                        }
                    }
                 });
