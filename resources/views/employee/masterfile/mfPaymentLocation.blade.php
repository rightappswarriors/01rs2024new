@if (session()->exists('employee_login'))     
  ...
  @section('title', 'Payment Location Master File')
  @section('content')
  <input type="text" id="CurrentPage" hidden="" value="AP014">
  <div class="content p-4">
      <div class="card">
          <div class="card-header bg-white font-weight-bold">
             Payment Location <span class="AP014_add"><a href="" title="Add New" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>  
          </div>
          <div class="card-body">
              ...
              <table class="table table-hover" id="example" style="overflow-x: scroll;" >
                <thead>
                  <tr>
                    ...
                    <th><center>Apply Location</center></th>
                    <th><center>Payment Facility</center></th>
          ...
          <div class="modal-body container" style=" background-color: #272b30;color: white;">
            ...
            <form id="NewFacServIn" class="container" action="#" class="row" data-parsley-validate>
              <div class="col-sm-12 alert alert-danger alert-dismissible fade show" style="display:none;margin:5px" id="AddErrorAlert" role="alert">
                ...
                <div class="row">
                  <div class="col-sm-4">Application Location:</div>
                  <div class="col-sm-8" style="margin:0 0 .8em 0;" >
                   <select id="apploc" data-parsley-required-message="*<strong>Apply Location</strong> required" class="form-control" required>  
                          <option value="">Select Application Location ...</option>
                          @if (isset($apploc))
                            @foreach ($apploc as $facilityagain)
                            	<option value="{{$facilityagain->applytoid}}">{{$facilityagain->applytoLoc}}</option>
                            @endforeach
                          @endif
                      </select>
                  </div>
                 ...
                 <div class="row">
                  <div class="col-sm-4">Payment Facility:</div>
                  <div class="col-sm-8" style="margin:0 0 .8em 0;" >
                   <select id="payfaci" data-parsley-required-message="*<strong>Payment Facility</strong> required" class="form-control" required>  
                          <option value="">Select Application Location ...</option>
                          @if (isset($paymentfaci))
                            @foreach ($paymentfaci as $facility)
								<option value="{{$facility->applylocid}}">{{$facility->applytofaci}}</option>
                            @endforeach
                          @endif
                      </select>
                  </div>
                ...
                <div class="col-sm-12">
                  <button type="submit" class="btn btn-outline-success form-control" style="border-radius:0;"><span class="fa fa-sign-up"></span>Add Facility Type</button>
                </div>
              </form>
          </div>
        </div>
  ...
  <div class="modal fade" id="IfActiveModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog" role="document">
        ...
        color: white;">
            <h5 class="modal-title text-center"><strong><span id="ifActiveTitle">Remove Facility Type</span></strong></h5>
            <hr>
            <div class="container">
              <form  id="AddOOP" class="row" data-parsley-validate>
                ...
                <div class="col-sm-12" id="Error">
                  ...
                  <p style="font-weight: bold;color:red">*Deleting may affect applications.</p>
  ...
  <script type="text/javascript">
  	...
  	function filterGroup(){
          var id = $('#filterer').val();
          var token = $('#token').val();
          var facidType = $("#FacServID").val();
          $.ajax({
                  url: " {{asset('employee/mf/get_facilitiesWithExtras')}}",
                  method: 'POST',
                  data: {
                    _token : token,
                    hfser_id : id,
                    ...
                    if (data == 'NONE') {
                      ...
                      for (var i = 0; i < data.length; i++) {
                        ...
                        $('#example').DataTable().row.add([
                              '<center>'+data[i].facname+'</center>',
                              '<center>'+data[i].applytoLoc+'</center>',
                              '<center>'+data[i].applytofaci+'</center>',
                              '<center><span class="AP014_cancel"><button class="btn btn-outline-danger" data-toggle="modal" data-target="#IfActiveModal" onclick="ShowDelete('+data[i].id+', \''+data[i].facname+'\', \''+data[i].hfser_id+'\')"><i class="fa fa-fw fa-trash"></i></button></span></center>'])
                        .draw();
                      }
                      GroupRightsActivate();
                    }
      ...
      $('#NewFacServIn').on('submit',function(event){
        ...
        var form = $(this);
        var apploc = $("#apploc").val();
        var payfaci = $("#payfaci").val();
        form.parsley().validate();
        if (form.parsley().isValid()) {
            $.ajax({
                    method: 'POST',
                    data: {_token:$('input[name="_token"]').val(),hfser_id:$('#appID').val(),facid:$('#FacServID').val(),apploc: apploc, payfaci: payfaci},
                    success: function(data) {
                      if (data) {
                          logActions('Added new Payment Location with ID: '+ data);
                          alert('Successfully Added Type Facility in an Application');
                          $("#myModal").modal('toggle');
                          $("#filterer").trigger('change');
                      } else if (data == 'SAME') {
                          alert('Facility Type is already in the selected Application');
                          $('#FacServID').focus();
                      } else if (data == 'ERROR') {
      ...
      $('#AddOOP').on('submit',function(event){
          ...
          if (form.parsley().isValid()) {
            $.ajax({
                url : '{{ asset('employee/mf/del_facilitiesWithOthers') }}',
                method : 'POST',
                data : {_token : $('#token').val(), id : $('#ToBeRemovedFacility').val()},
                success : function(data){
                  if (data == 'DONE') {
                    logActions('Deleted Manage Facility with ID: '+$('#ToBeRemovedFacility').val());
