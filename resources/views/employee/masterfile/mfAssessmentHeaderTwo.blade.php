@if (session()->exists('employee_login'))	
	...
	@section('title', 'Assessment Header Two Master File')
	@section('content')
		...
		<datalist id="rgn_list">
			@if (isset($allData))
				@foreach ($allData as $hfstype)
					<option value="{{$hfstype->asmtH2ID}}">{{$hfstype->h1name}}</option>
				@endforeach
			@endif
		</datalist>
		<div class="card">
			<div class="card-header bg-white font-weight-bold">
			Assessment Header Two <span class="header_add"><a href="#" title="Add New Assessment Header Two" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>
				...
				<table class="table display" id="example" style="overflow-x: scroll;" >
					<thead>
						<tr>
							...
							<th class="text-center">Header Level 1 Description</th>
						...
						@if (!empty($allData))
							@foreach($allData as $titleData)
								<tr class="text-center">
									...
									<td>
										<button type="button" class="btn btn-outline-warning" onclick="showData('{{$titleData->asmtH2ID}}', '{{$titleData->h2name}}', '{{$titleData->asmtH1ID}}', '{{$titleData->isdisplay}}');" data-toggle="modal" data-target="#GodModal"><i class="fa fa-fw fa-edit"></i></button>
										<button type="button" class="btn btn-outline-danger" onclick="showDelete('{{$titleData->asmtH2ID}}', '{{$titleData->h2name}}');" data-toggle="modal" data-target="#DelGodModal"><i class="fa fa-fw fa-trash"></i></button>
									</td>
								</tr>
							@endforeach
						@endisset
	...
	<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content" style="border-radius: 0px;border: none;">
				...
				color: white;">
					<h5 class="modal-title text-center"><strong>Add New Assessment Header Two</strong></h5>
					<hr>
					<div class="container">
						<form id="addRgn" class="row"  data-parsley-validate>
							...
							<div class="col-sm-8" style="margin:0 0 .8em 0;">
								<input type="text" id="new_rgnid" data-parsley-required-message="*<strong>Header Two Code</strong> required"  class="form-control"  required>
							</div>
							...
							<div class="col-sm-8" style="margin:0 0 .8em 0;">
								<select name="newserv" id="newserv" class="form-control" data-parsley-required-message="*<strong>Part name</strong> required" required="">
									...
									@foreach($part as $se)
									<option value="{{$se->asmtH1ID}}">{{$se->h1name}}</option>
									@endforeach
								</select>
							</div>
							<div class="col-sm-4">Show on Assessment?:</div>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content" style="border-radius: 0px;border: none;">
				<div class="modal-body" style=" background-color: #272b30;color: white;">
					<h5 class="modal-title text-center"><strong>Edit Assessment Header Two</strong></h5>
					<hr>
					<div>
	...
	<script type="text/javascript">
		...
		function showData(id,desc,serv,isAssess){
	      $('#EditBody').empty();
	      $('#EditBody').append(
							...
							{{-- <div class="row"> --}}
								'<div class="col-md">'+
									'<label class="form-check-label" for="exampleRadios1Edit">'+
										'Yes'+
										'<input type="radio" class="form-control" id="exampleRadios1Edit" name="isAssessEdit" value="1">'+
									'</label>'+
								'</div>'+
							{{-- </div> --}}
						'</div>'+
							...
							{{-- <div class="row"> --}}
								'<div class="col-md">'+
									'<label class="form-check-label" for="exampleRadios2Edit">'+
										'No'+
										'<input type="radio" class="form-control" id="exampleRadios2Edit" name="isAssessEdit" value="0">'+
									'</label>'+
								'</div>'+
							{{-- </div> --}}
						'</div>'+
	            ...
	            if (test == -1) { // Not in Array
	                $.ajax({
	                  ...
	                  data: {
	                    ...
	                    serv :  $('#newserv').val(),
	                    isAssess : $('input[name=isAssess]:checked').val(),
	                  ...
	                  success: function(data) {
	                    if (data == 'DONE') {
	                        alert('Successfully Added New Assessment Header Two');
	                        location.reload();
	                    } else if(data == 'ERROR'){
	      ...
	      event.preventDefault();
	        ...
	        form.parsley().validate();
	         if (form.parsley().isValid()) {
	           ...
	           $.ajax({
	              method: 'POST',
	              data : {_token:$('#token').val(),id:x,name:y,filename:a,serv: $("#newserv_edit").val(), isAssess: $('input[name=isAssessEdit]:checked').val(),action:'edit'},
	              success: function(data){
	                  if (data == "DONE") {
