@if (session()->exists('employee_login'))  
  ...
  @section('title', 'Mornitoring Correction')
  @section('content')
  ...
  <div class="content p-4">
      ...
      /
      <a href="{{asset('employee/dashboard/processflow/complianceattachment/')}}/{{$monid}}{{ app('request')->input('from') ? '/?from=rec'  : '' }}"> Attachment</a> / 
      <a href="{{asset('employee/dashboard/processflow/complianceremarks/')}}/{{$monid}}{{ app('request')->input('from') ? '/?from=rec'  : ''}}"> Remarks </a> / 
          ...
          <div class="card-body table-responsive">
          	...
          	<table class="table table-bordered table-striped dataTable" style="font-size:13px;" id="example">
                  ...
                  <tr>
                      ...
                      <td scope="col" class="text-center">For Correction</td>
                   ...
                   @if (isset($BigData))
                      @foreach ($BigData as $index => $data)
                          ...
                          <tr>
                            ...
                            <td class="text-center">{!!$data->novdesc!!}</td>
              
                            <td>

                            @if ( request()->has('from') )
                            <input type="checkbox" value="0" disabled class="complianceChecker" {{$data->status == 0 ? 'checked' : '' }}> No
                            <input type="checkbox" value="1" disabled class="complianceChecker compliance_complied" {{$data->status == 1 ? 'checked' : '' }} > Yes
                            
                            @else
                            <input type="checkbox" value="0" class="complianceChecker" {{$data->status == 0 ? 'checked' : '' }} onclick="complianceChecker({{$data->novid}}, 0, {{$data->novid}})"> No
                            <input type="checkbox" value="1" class="complianceChecker compliance_complied" {{$data->status == 1 ? 'checked' : '' }} onclick="complianceChecker({{$data->novid}}, 1, {{$data->novid}})"> Yes
                            @endif
                            </td>
              ...
              @else
                ...
                <div class="" style="display:inline-block;">
                  <button type="button" onclick="complied(0, {{$monid}})" class="btn btn-danger w-100">
                    <i class="fa fa-plus" aria-hidden="true"></i> &nbsp;
                    For Denial
                  </button>
                </div>
                              ...
                              <div class="container">
                                <form id="compliance_form">
                                  <div class="col-md-12">
                                    ...
                                    <input type="hidden" name="compliance_id" value="{{$monid}}">
